function LogsTeacher() {
    ...
    this.fetchLogs = async() => {
        const response = await fetch("../index.php?action=visitorLogs&view", {
    ...
    this.searchLogs = () => {
        searchVisitorLogsData.oninput = async(e) => {
            ...
            if(count > 0) {
                ...
                let response = await fetch(`../index.php?action=visitorLogs`, {
                  credentials: "same-origin",
                  method: 'POST',
                  body: formData
                });
    ...
    this.limitLogPages = () => {
        limitvisitorlogsnum.onchange = async(e) => {
            let val = e.currentTarget.value;
            
            const response = await fetch(`../index.php?action=visitorLogs&logsort=${val}`, {
                credentials: "same-origin",
                method: "GET",
            });
            let { message, status} = await response.json();
    ...
    this.totalPagesLogs= async() => {
        const response = await fetch(`../index.php?action=visitorLogs&totalpage`, {
            credentials: "same-origin",
            method: "GET",
        });
        ...
        if(status == 'success') {
            ...
            let pagenumber = 1;
            logsVisitorPrev.onclick = async() => {
                if(pagenumber >= 1) {
                    this.paginationLogs(pagenumber -= 1);
                } 

                let btnPrev = document.querySelector(`#btnListVisitor${pagenumber +1}`);
            ...
            logsVisitorNext.onclick = async() => {
                if(pagenumber < total_pages) {
                    this.paginationLogs(pagenumber += 1);
                }

                let btnNext = document.querySelector(`#btnListVisitor${pagenumber}`);
            ...
            for (let i = 1; i < total_pages+1; i++) {
                output += `
                    <div id="btnListVisitor${i}" onclick="(logVisitorObj.paginationLogs(${i}))" class="child p-1 mr-2 rounded-md cursor-pointer w-16 h-14 border-solid border-2 flex items-center justify-center border-gray-400 hover:bg-blue-100 font-bold">${i}</div>
                `;
            }
            
            document.querySelector('#logs-btn-visitor-pages').innerHTML = output;
    ...
    this.paginationLogs = async(pagenum) => {
        const response = await fetch(`../index.php?action=visitorLogs&pagenum=${pagenum}`, {
            credentials: "same-origin",
            method: "GET",
        });
        let { message, status} = await response.json();
    ...
    this.trackDateLogs = () => {
        ...
        datevisitorform.onchange = async(e) => {
            this.activeDropown(
                'datevisitorto',
                "rgb(60, 179, 113)",
                false
        ...
        datevisitorto.onchange = async(e) => {
            let datefrom = document.querySelector('#datevisitorform').value;
            let datevisitorto  = e.currentTarget.value;

            if(datefrom.length > 0) {
                let data = {
                    datefrom: datefrom,
                    dateto: datevisitorto
                };
                let formData = new FormData();
    ...
    this.printAllLogsData = () => {
        btnPrintVisitorsLogsData.onclick = async(e) => {
            swal({
    ...
    this.tableBodyLogs = (data) => {
        ...
        document.querySelector("#tbodyvisitorlogs").innerHTML = row;
    };
}

let logVisitorObj = new LogsTeacher();
logVisitorObj.fetchLogs();
logVisitorObj.limitLogPages()
logVisitorObj.totalPagesLogs();
logVisitorObj.trackDateLogs();
logVisitorObj.printAllLogsData();
logVisitorObj.searchLogs();
